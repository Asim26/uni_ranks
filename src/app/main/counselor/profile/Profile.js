...
export default function Profile() {
	...
	useEffect(() => {
		fetch(api + 'counselors?userId=' + id).then(res => res.json()).then(results => {
	...
	function handleSubmit(model) {
		fetch(api + 'updateCounselorPr', { method: 'PUT', body: '{"id": "' + id + '", "name": "' + model.name + '", "pass": "' + model.password + '", "phone": "' + model.phone + '", "email": "' + model.email + '", "title": "' + model.title + '", "address": "' + model.address + '"}', headers: { 'Content-Type': 'application/json' } }).then(res => res.text());
			...
			header={
				<div className="pt-48 px-24 w-full flex">
					...
					<div className="w-full">
						<h1 className="text-48">Welcome, {title}. {name}</h1>
					</div>
					<div className="pt-48">
